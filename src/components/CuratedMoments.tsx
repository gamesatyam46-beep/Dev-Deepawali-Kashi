...
export const CuratedMoments: React.FC<CuratedMomentsProps> = ({ onOpenBooking }) => {
  ...
  const getIcon = (name: string) => {
    switch (name) {
      case 'Flame': return <Flame className="w-4 h-4 sm:w-5 sm:h-5 text-[#f2ca50]" />;
      case 'Sparkles': return <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-[#f2ca50]" />;
      case 'Zap': return <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-[#f2ca50]" />;
      case 'Ship': return <Ship className="w-4 h-4 sm:w-5 sm:h-5 text-[#f2ca50]" />;
      default: return <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-[#f2ca50]" />;
    }
  };

  return (
    <section id="experiences" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-[#070a10] relative">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 4 Cards Grid: Single column vertical stack on mobile (<768px), 2-col on tablet, 4-col on desktop */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
          {CURATED_MOMENTS.map((moment) => (
            <div
              key={moment.id}
              onClick={() => setActiveMoment(moment)}
              className="group relative min-h-[380px] sm:min-h-[420px] rounded-2xl overflow-hidden border border-[#d4af37]/30 bg-[#0d121d] shadow-[0_10px_30px_rgba(0,0,0,0.8)] cursor-pointer transform transition-all duration-300 hover:-translate-y-1.5 hover:border-[#d4af37] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] flex flex-col justify-between"
            >
              {/* Background Image */}
              <img
                src={moment.image}
              ...
              <div className="relative top-0 left-0 right-0 p-4 flex items-center justify-between z-10">
                <span className="px-3 py-1 rounded-full text-[10px] font-semibold tracking-wider uppercase bg-[#0d121d]/90 border border-[#d4af37]/40 text-[#ffe088] backdrop-blur-md">
                  {moment.badge}
                </span>
              ...
              {/* Bottom Content */}
              <div className="relative p-5 sm:p-6 z-10 space-y-2 mt-auto">
                <div className="text-xs text-[#ffe088] font-serif">{moment.hindiTitle}</div>
                <h3 className="font-serif text-xl sm:text-2xl font-bold text-[#f2ca50] group-hover:text-[#ffe088] transition-colors leading-snug">
                  {moment.title}
                </h3>
                ...
                <div className="pt-2 flex items-center justify-between text-xs text-[#ffe088] font-medium border-t border-[#353535] opacity-90 group-hover:opacity-100 transition-opacity">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
            <div className="relative w-full max-w-2xl bg-[#0d121d] border border-[#d4af37]/60 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(212,175,55,0.4)] max-h-[90vh] overflow-y-auto">
              ...
              <div className="relative h-52 sm:h-64 overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0d121d] via-transparent to-black/50" />
                <button
                  onClick={() => setActiveMoment(null)}
                  className="absolute top-4 right-4 w-9 h-9 rounded-full bg-black/70 border border-[#d4af37]/40 flex items-center justify-center text-[#e5e2e1] hover:text-[#f2ca50] transition-colors cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="p-5 sm:p-8 space-y-4">
                <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-[#d0c5af]">
                ...
                <p className="text-xs sm:text-base text-[#d0c5af] leading-relaxed">
                ...
                <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-[#1e2535]">
                  <div className="text-xs text-[#8e9cb4] text-center sm:text-left">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-6 py-3 rounded-full bg-gradient-to-r from-[#25D366] via-[#20BA5A] to-[#128C7E] text-white font-semibold text-xs sm:text-sm tracking-wide shadow-[0_0_20px_rgba(37,211,102,0.4)] hover:shadow-[0_0_30px_rgba(37,211,102,0.7)] transition-all cursor-pointer"
                  >
