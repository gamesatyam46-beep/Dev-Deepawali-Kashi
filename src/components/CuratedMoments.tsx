...
export const CuratedMoments: React.FC<CuratedMomentsProps> = ({ onOpenBooking }) => {
  ...
  return (
    <section id="experiences" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#070a10] relative">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 4 Cards Grid from Mockup */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {CURATED_MOMENTS.map((moment, index) => {
            const isOffset = index % 2 === 1;
            return (
              <div
                key={moment.id}
                onClick={() => setActiveMoment(moment)}
                className={`group relative h-[420px] rounded-2xl overflow-hidden border border-[#d4af37]/30 bg-[#0d121d] shadow-[0_10px_30px_rgba(0,0,0,0.8)] cursor-pointer transform transition-all duration-500 hover:-translate-y-2 hover:border-[#d4af37] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] ${
        ...
        {activeMoment && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
            <div className="relative w-full max-w-2xl bg-[#0d121d] border border-[#d4af37]/60 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(212,175,55,0.4)]">
              {/* Header Image */}
              <div className="relative h-64 overflow-hidden">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 w-9 h-9 rounded-full bg-black/60 border border-[#d4af37]/40 flex items-center justify-center text-[#e5e2e1] hover:text-[#f2ca50] transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="p-6 sm:p-8 space-y-4">
                ...
                <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-[#1e2535]">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-6 py-2.5 rounded-full bg-gradient-to-r from-[#25D366] via-[#20BA5A] to-[#128C7E] text-white font-semibold text-xs sm:text-sm tracking-wide shadow-[0_0_20px_rgba(37,211,102,0.4)] hover:shadow-[0_0_30px_rgba(37,211,102,0.7)] transition-all"
                  >
