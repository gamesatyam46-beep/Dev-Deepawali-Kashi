...
import { Camera, Sparkles, X, Maximize2, MapPin } from 'lucide-react';
...
export const GallerySection: React.FC = () => {
  ...
  return (
    <section id="gallery" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#070a10] relative border-t border-[#d4af37]/20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <div
              ...
              className="group relative h-80 rounded-2xl overflow-hidden border border-[#d4af37]/30 hover:border-[#d4af37] cursor-pointer shadow-[0_10px_30px_rgba(0,0,0,0.8)] transition-all duration-300 transform hover:-translate-y-1 bg-[#0d121d]"
            >
              <img
                src={item.image}
                alt={item.title}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#070a10] via-[#070a10]/40 to-transparent opacity-85 group-hover:opacity-95 transition-opacity" />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 space-y-1">
                ...
                <h3 className="font-serif text-lg font-bold text-[#e5e2e1] group-hover:text-[#ffe088] transition-colors">
                  {item.title}
                </h3>
        ...
        {lightboxItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
            <div className="relative max-w-4xl w-full bg-[#0d121d] border border-[#d4af37]/60 rounded-2xl overflow-hidden shadow-[0_0_60px_rgba(212,175,55,0.4)]">
              <div className="relative max-h-[70vh] overflow-hidden bg-black">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/70 border border-[#d4af37]/40 flex items-center justify-center text-[#e5e2e1] hover:text-[#f2ca50] transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>

              <div className="p-6 bg-[#0d121d] border-t border-[#1e2535] flex items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 text-xs text-[#f2ca50] mb-1">
                    <MapPin className="w-3.5 h-3.5" />
                    <span>{lightboxItem.location}</span>
                  </div>
                  <h3 className="font-serif text-xl font-bold text-[#ffe088]">
                    {lightboxItem.title}
                  </h3>
