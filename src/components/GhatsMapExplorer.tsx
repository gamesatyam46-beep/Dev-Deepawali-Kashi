...
import { Compass, Flame, Clock, MapPin, Sparkles, ChevronRight, BookOpen, Layers } from 'lucide-react';

export const GhatsMapExplorer: React.FC = () => {
  const [selectedGhat, setSelectedGhat] = useState<GhatInfo>(GHATS_DATA[2]); // Default Dashashwamedh

  return (
    <section id="ghats" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#0a0e17] relative overflow-hidden border-t border-[#d4af37]/20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          ...
          <p className="font-sans text-sm sm:text-base text-[#d0c5af]">
            Follow our luxury vessel's private cruising path along the 7-kilometer crescent from South Kashi to North Kashi, stopping at the most sacred sanctums on Dev Deepawali night.
          </p>
        </div>
          ...
          <div className="flex items-center justify-between min-w-[760px] relative px-4">
            ...
            {GHATS_DATA.map((ghat, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center font-serif text-sm font-bold border-2 transition-all duration-300 ${
                      isSelected
        ...
        <div className="bg-[#101624]/95 border border-[#d4af37]/40 rounded-2xl overflow-hidden shadow-[0_15px_40px_rgba(0,0,0,0.9)] grid grid-cols-1 lg:grid-cols-12 backdrop-blur-xl">
          {/* Left Ghat Image */}
          <div className="lg:col-span-6 relative min-h-[320px] lg:min-h-[440px] overflow-hidden">
            <img
            ...
            <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-xs text-[#ffe088] bg-black/75 px-4 py-2 rounded-xl backdrop-blur-md border border-[#d4af37]/30">
              <span className="flex items-center gap-1.5 font-medium">
                <Flame className="w-4 h-4 text-[#f2ca50]" />
                {selectedGhat.diyaCountApprox}
              </span>
              <span className="flex items-center gap-1.5 font-medium">
                <Clock className="w-4 h-4 text-[#f2ca50]" />
                {selectedGhat.aartiTime}
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 p-6 sm:p-8 flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <h3 className="font-serif text-3xl font-bold text-[#f2ca50]">
                  {selectedGhat.name}
                </h3>
            ...
            <div className="p-3.5 rounded-xl bg-[#0a0e17]/90 border border-[#d4af37]/30 flex items-center justify-between text-xs">
